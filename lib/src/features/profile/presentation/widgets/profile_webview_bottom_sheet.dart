...
class _ProfileWebViewBottomSheetState
  ...
  Future<void> _injectScrollListener() async {
    ...
    const script = '''
      (function() {
        if (!window.SnaplookScrollState || window.__snaplookScrollListenerAdded) return;
        window.__snaplookScrollListenerAdded = true;
        function sendScrollPosition() {
          var position = window.scrollY || 0;
          try {
            window.SnaplookScrollState.postMessage(String(position));
          } catch (err) {}
        }
