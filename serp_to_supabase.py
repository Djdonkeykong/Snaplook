...
def run_search(query: str, api_key: str, num: int):
    ...
    if "error" in results:
        ...
        return []
    images = results.get("images_results", [])[:num]  # enforce max len locally
    if not images:
        meta = results.get("search_metadata", {}) or {}
        print(f"No images_results returned. SerpApi status: {meta.get('status')} url: {meta.get('google_images_url')}")
    return images
...
def build_rows(
    ...
    for img in images:
        ...
        domain_ok = allowed_domains == ["*"] or (is_allowed(link, allowed_domains) or is_allowed(original, allowed_domains))
        if not domain_ok:
            stats["domain_fail"] += 1
            continue
...
def main():
    ...
    parser.add_argument("--domains", default="shop.mango.com", help="Comma-separated allowed domains; use * to allow any")
    parser.add_argument("--num", type=int, default=20, help="Number of image results to fetch (default 20)")
